...
SYSCALL_DEFINE3(open, const char __user *, filename, int, flags, umode_t, mode)
{
	...
	if (ret>=0){
		struct pid_node *cur;
		printk(KERN_INFO "[resource_tracker] PID %d is already being tracked op 1.\n", pid)
		...
		list_for_each_entry(cur,&tracked_resources_list,next_prev_list){
            if (cur->proc_resource->pid == current->pid) {
				printk(KERN_INFO "[resource_tracker] PID %d is already being tracked op 2.\n", pid)
		...
		umode_t, mode)
{
	...
	if (ret>=0){
		struct pid_node *cur;
		printk(KERN_INFO "[resource_tracker] PID %d is already being tracked o1 1.\n", pid)
		write_lock(&resource_tracker_lock);
		list_for_each_entry(cur,&tracked_resources_list,next_prev_list){
            if (cur->proc_resource->pid == current->pid) {
				printk(KERN_INFO "[resource_tracker] PID %d is already being tracked o1 2.\n", pid)
                cur->proc_resource->openfile_count++;
                break;
			}
		}
		...
		struct open_how __user *, how, size_t, usize)
{
	...
	if (ret>=0){
		struct pid_node *cur;
		printk(KERN_INFO "[resource_tracker] PID %d is already being tracked o2 1.\n", pid)
		write_lock(&resource_tracker_lock);
		list_for_each_entry(cur,&tracked_resources_list,next_prev_list){
            if (cur->proc_resource->pid == current->pid) {
				printk(KERN_INFO "[resource_tracker] PID %d is already being tracked o2 2.\n", pid)
                cur->proc_resource->openfile_count++;
                break;
			}
		}
...
SYSCALL_DEFINE2(creat, const char __user *, pathname, umode_t, mode)
{
	...
	if (ret>=0){
		struct pid_node *cur;
		printk(KERN_INFO "[resource_tracker] PID %d is already being tracked cr 1.\n", pid)
		write_lock(&resource_tracker_lock);
		list_for_each_entry(cur,&tracked_resources_list,next_prev_list){
            if (cur->proc_resource->pid == current->pid) {
				printk(KERN_INFO "[resource_tracker] PID %d is already being tracked cr 2.\n", pid)
                cur->proc_resource->openfile_count++;
                break;
			}
		}
